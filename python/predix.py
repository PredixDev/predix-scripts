...
def createPredixUAASecurityService(config):
	...
	uaaJsonrequest = "cf cs "+config.predixUaaService+" "+config.predixUaaServicePlan +" "+config.rmdUaaName+ " -c " + '"'+os.path.join(os.getcwd(),uaa_payload_filename)+'"'
...
def createBindPredixACSService(config, rmdAcsName):
	...
	#create UAA instance
	acsJsonrequest = "cf cs "+config.predixAcsService+" "+config.predixAcsServicePlan +" "+rmdAcsName+ " -c "+ '"'+os.path.join(os.getcwd(),acs_payload_filename)+'"'
	...
	if statementStatus == 1 :
		sys.exit("Error binding ACS service instance to boot ")
...
def removemvntestfile():
	from os.path import expanduser
	homeDir = expanduser("~")
	pomFile = os.path.join(homeDir, ".m2", "repository", "com", "ge", "predix", "solsvc", "ext-api", "2.0.5", "ext-api-2.0.5.pom")
	if (os.path.exists(pomFile)):
		os.remove(pomFile)

def assertmvn():
	removemvntestfile()
	out, err, exitcode = call("mvn org.apache.maven.plugins:maven-dependency-plugin:2.10:get -DrepoId=predix.repo -DrepoUrl=https://artifactory.predix.io/artifactory/PREDIX-EXT -Dartifact=com.ge.predix.solsvc:ext-api:2.0.5:pom")
	if (out.find("nodename nor servname provided") > -1):
		print ("Unable to connect to maven repository at https://artifactory.predix.io")
		print ("Verify that you are connected to the internet.")
		print ("If you are behind a corporate proxy, verify that you have entered valid proxy settings in your maven settings file.")
		print ("The maven settings file is located at ~/.m2/settings.xml.")
		sys.exit(1)
	if (out.find("ReasonPhrase:Unauthorized") > -1):
		print ("Failed to fetch artifact from https://artifactory.predix.io")
		print ("Verify that you have provided a valid username/password in your maven settings file.")
		print ("The maven settings file is located at ~/.m2/settings.xml.")
		sys.exit(1)
