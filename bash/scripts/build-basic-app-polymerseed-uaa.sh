...
# ********************************** MAIN **********************************
function build-basic-app-polymerseed-uaa-main() {
  ...
  __find_and_replace "- name: .*" "- name: $FRONT_END_POLYMER_SEED_UAA_APP_NAME" "manifest.yml" "$logDir"
  ...
  __append_new_head_log "Deploying the application \"$FRONT_END_POLYMER_SEED_UAA_APP_NAME\"" "-" "$logDir"
  if px push; then
  ...
  if [[ $SKIP_BROWSER == 0 ]]; then
    apphost=$(px app $FRONT_END_POLYMER_SEED_UAA_APP_NAME | grep urls: | awk '{print $2;}')
    case "$(uname -s)" in
       Darwin)
         # OSX
         open https://$apphost
  ...
  if __echo_run px start $FRONT_END_POLYMER_SEED_UAA_APP_NAME; then
    __append_new_line_log "$FRONT_END_POLYMER_SEED_UAA_APP_NAME started!" "$logDir" 1>&2
  else
    __error_exit "Couldn't start $FRONT_END_POLYMER_SEED_UAA_APP_NAME" "$logDir"
  fi
  ...
  echo "Predix UI Polymer Starter with UAA based on Node.js Express Web App"  >> $SUMMARY_TEXTFILE
  echo "--------------------------------------------------"  >> $SUMMARY_TEXTFILE
  echo "Installed a front-end named $FRONT_END_POLYMER_SEED_UAA_APP_NAME and updated the property files and manifest.yml with UAA info" >> $SUMMARY_TEXTFILE
  echo "Setup localconfig.json which is used when developing locally on your laptop" >> $SUMMARY_TEXTFILE
  echo "" >> $SUMMARY_TEXTFILE
  echo "Front-end App URL: https://$FRONT_END_POLYMER_SEED_UAA_APP_NAME.run.$CLOUD_ENDPONT" >> $SUMMARY_TEXTFILE
  echo "Front-end App Login: app_user_1/app_user_1" >> $SUMMARY_TEXTFILE
  echo "" >> $SUMMARY_TEXTFILE
  echo -e "You can execute 'px env "$FRONT_END_POLYMER_SEED_UAA_APP_NAME"' to view info about your front-end app, UAA" >> $SUMMARY_TEXTFILE
