...
function build-basic-app-polymerseed-uaa-main() {
  ...
  if [[ $SKIP_BROWSER == 0 ]]; then
    apphost=$(cf app $FRONT_END_POLYMER_SEED_UAA_APP_NAME | grep urls: | awk '{print $2;}')
    case "$(uname -s)" in
       Darwin)
         # OSX
         open https://$apphost
  ...
  if __echo_run cf start $FRONT_END_POLYMER_SEED_UAA_APP_NAME; then
  ...
  echo "" >> $SUMMARY_TEXTFILE
  echo -e "You can execute 'cf env "$FRONT_END_POLYMER_SEED_UAA_APP_NAME"' to view info about your front-end app, UAA" >> $SUMMARY_TEXTFILE
