...
function edge-starter-kit-device-login-main() {
  ...
  TARGETDEVICEIP=""
    ...
    __echo_run ssh $TARGETDEVICEUSER@$TARGETDEVICEIP 'pwd; rm -rf kit-cloud/predix-scripts; export DYLD_INSERT_LIBRARIES=; bash -l <( curl https://raw.githubusercontent.com/PredixDev/kit-cloud-app/develop/scripts/quickstart-kit-cloud-app.sh) -o -kitpca --skip-setup -i' $INSTANCE_PREPENDER
    pwd
  else
