...
FILES_HELPER_LogDir="$FILES_HELPER_PATH/../log"
...
{
	__validate_num_arguments 4 $# "\"__find_and_replace()\" expected in order:  Pattern to find, String relacing the matching pattern, filename, path of where to generate log" "$FILES_HELPER_LogDir"
	#echo sed "s#$1#$2#" "$3"
	sed "s#$1#$2#" "$3" > "$3.tmp"
	if mv "$3.tmp" "$3"; then
...
{
	__validate_num_arguments 4 $# "\"__find_and_append_new_line()\" expected in order:  Pattern to find, String of the new line being appended, filename, path of where to generate log" "$FILES_HELPER_LogDir"
...
{
	__validate_num_arguments 5 $# "\"__find_and_replace()\" expected in order:  Pattern to find, String relacing the matching pattern, filename, path of where to generate log" "$FILES_HELPER_LogDir"
	#echo sed "s#$1#$2#" "$3"
	sed "s#$1#$2#" "$3" > "$3.tmp"
	if mv "$3.tmp" "$5"; then
		echo "Successfully ran sed command on file: \"$3\", replacing pattern: \"$1\", with: \"$2\""
...
{
	__validate_num_arguments 3 $# "\"__append_new_line_log()\" expected in order: String of the new line being appended, seperater char, path of where to generate log" "$FILES_HELPER_LogDir"
	__print_center "$1" "$2"
	echo $(timestamp): " --- " "$1"  >> "$3/quickstartlog.log"
}
...
{
	__validate_num_arguments 2 $# "\"__append_new_line_log()\" expected in order: String of the new line being appended, path of where to generate log" "$FILES_HELPER_LogDir"
	echo "-->> $1"
...
{
	__validate_num_arguments 2 $# "\"__append_new_line()\" expected in order: String of the new line being appended, filename" "$FILES_HELPER_LogDir"
	echo -e "$1" >> "$2"
}
...
function __checkoutTags
{
	...
	if [[ ( "$1" != "") ]]; then
		...
		repo_version="$(echo "$a" | sed -n "/$reponame/p" $FILES_HELPER_PATH/../../version.json | awk -F"\"" '{print $4}' | awk -F"#" '{print $NF}')"
...
function getRepoURL {
	local  repoURLVar=$2
	reponame=$(echo "$1" | awk -F "/" '{print $NF}')
	url=$(echo "$a" | sed -n "/$reponame/p" $FILES_HELPER_PATH/../../version.json | awk -F"\"" '{print $4}' | awk -F"#" '{print $1}')
	eval $repoURLVar="'$url'"
}
