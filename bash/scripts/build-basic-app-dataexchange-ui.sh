...
# ********************************** MAIN **********************************
function build-basic-app-dataexchange-ui-main() {
  __validate_num_arguments 1 $# "\"build-basic-app-dataexchange-ui.sh\" expected in order: String of Predix Application used to get VCAP configurations" "$logDir"
  ...
  getGitRepo "data-exchange-ui"
  cd data-exchange-ui

  #     Edit the manifest.yml file
  #     Modify the name of the application
  __find_and_replace "- name: .*" "- name: $FRONT_END_DATAEXCHANGE_UI_APP_NAME" "manifest.yml" "$logDir"
  ...
  __find_and_replace "\#clientId: .*" "clientId: $UAA_CLIENTID_LOGIN" "manifest.yml" "$logDir"
  __find_and_replace "\#base64ClientCredential: .*" "base64ClientCredential: $MYLOGINGENRICS_SECRET" "manifest.yml" "$logDir"

  #    Set the service clientid and base64ClientCredentials
  __find_and_replace "\#serviceClientId: .*" "serviceClientId: $UAA_CLIENTID_GENERIC" "manifest.yml" "$logDir"
  __find_and_replace "\#serviceBase64ClientCredential: .*" "serviceBase64ClientCredential: $MYGENERICS_SECRET" "manifest.yml" "$logDir"
...
#    Set the service clientid label
  ...
  __find_and_replace ".*clientId\":.*" "    \"clientId\": \"$UAA_CLIENTID_LOGIN\"," "server/localConfig.json" "$logDir"
  ...
  __append_new_head_log "Deploying the application \"$FRONT_END_DATAEXCHANGE_UI_APP_NAME\"" "-" "$logDir"
  if cf push; then
  ...
  if [[ $SKIP_BROWSER == 0 ]]; then
    apphost=$(cf app $FRONT_END_DATAEXCHANGE_UI_APP_NAME | grep urls: | awk '{print $2;}')
    case "$(uname -s)" in
       Darwin)
         # OSX
         open https://$apphost
  ...
  if __echo_run cf start $FRONT_END_DATAEXCHANGE_UI_APP_NAME; then
    __append_new_line_log "$FRONT_END_DATAEXCHANGE_UI_APP_NAME started!" "$logDir" 1>&2
  else
    __error_exit "Couldn't start $FRONT_END_DATAEXCHANGE_UI_APP_NAME" "$logDir"
  fi
  ...
  echo "Data Exchange UI based on Polymer Starter WebApp"  >> $SUMMARY_TEXTFILE
  echo "--------------------------------------------------"  >> $SUMMARY_TEXTFILE
  echo "Installed a front-end named $FRONT_END_DATAEXCHANGE_UI_APP_NAME and updated the property files and manifest.yml" >> $SUMMARY_TEXTFILE
  ...
  echo "" >> $SUMMARY_TEXTFILE
  echo -e "You can execute 'cf env "$FRONT_END_DATAEXCHANGE_UI_APP_NAME"' to view info about your front-end app" >> $SUMMARY_TEXTFILE
